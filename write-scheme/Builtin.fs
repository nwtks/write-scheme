...
module Builtin =
    ...
    type private SBinding =
        | SingleB of SExpression
        | EllipsisB of SExpression list

    let private mergeBindings (b1: Map<string, SBinding>) (b2: Map<string, SBinding>) =
        Map.fold (fun acc k v -> Map.add k v acc) b1 b2

    let rec private collectPatternVars (literals: Set<string>) =
        function
        | SSymbol "_" -> []
        | SSymbol "..." -> []
    ...
    let rec private collectTemplateVars =
        function
        | SSymbol s -> [ s ]
    ...
    let rec private expandTemplate (bindings: Map<string, SBinding>) =
        function
        | SSymbol s ->
    ...
    and private expandTemplateList (bindings: Map<string, SBinding>) =
        function
        | [] -> []
        | tmpl :: SSymbol "..." :: rest ->
